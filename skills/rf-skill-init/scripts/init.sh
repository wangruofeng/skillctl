...
# 同时确保项目根存在 CLAUDE.md，并将 AGENTS.md 软链接到 CLAUDE.md
set -euo pipefail
...
初始化项目 skill 目录配置，以 .claude/skills 为基准（唯一事实源）：
  1. 确保 CLAUDE.md 存在；AGENTS.md 软链接到 CLAUDE.md
...
is_agent_docs_ready() {
  # CLAUDE.md 必须是普通文件（不是断链）；AGENTS.md 必须指向 CLAUDE.md
  [[ -f "$CLAUDE_MD" && ! -L "$CLAUDE_MD" ]] || return 1
  [[ -L "$AGENTS_MD" ]] || return 1
  [[ "$(readlink "$AGENTS_MD")" == "$AGENTS_LINK_TARGET" ]] || return 1
  return 0
}
...
# 确保 CLAUDE.md 存在，AGENTS.md -> CLAUDE.md
ensure_agent_docs() {
  echo "▶ Agent 文档"
