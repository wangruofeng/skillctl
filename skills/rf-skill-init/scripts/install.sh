...
MARKER_BEGIN="# >>> skills-init (rf-skill-init) >>>"
MARKER_END="# <<< skills-init (rf-skill-init) <<<"
# 历史 marker（重命名前 / 错配块），安装与卸载时一并清理
LEGACY_MARKERS=(
  "# >>> skills-init (rf-skills-init) >>>|# <<< skills-init (rf-skills-init) <<<"
  "# >>> skills-sync (rf-skills-init) >>>|# <<< skills-sync (rf-skills-init) <<<"
)
